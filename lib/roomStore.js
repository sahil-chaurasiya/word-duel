...
function createRoom(hostId, hostName) {
  ...
  rooms[code] = {
    ...
    // Pusher trigger fn injected at call time
    pusherTrigger: null
  };
  return rooms[code];
}

// pusherTrigger: async (channel, event, data) => void
async function startRound(room, pusherTrigger) {
  ...
  room.roundTimerHandle = setTimeout(() => {
    resolveRound(room, pusherTrigger);
  }, (duration + 0.5) * 1000);

  // Bot move
  if (room.isBot) {
    const { getBotMove } = require('./botLogic');
    const { word, delay } = getBotMove(
      room.category, room.botDifficulty, room.usedWords, duration
    );
    const actualDelay = Math.min(delay, duration * 1000 - 500);
    setTimeout(() => {
      if (rooms[code] && room.status === 'playing' && !room.roundSubmissions['BOT']) {
        room.roundSubmissions['BOT'] = {
          word, time: Date.now() - room.roundStartTime
        };
        const humanId = room.playerOrder.find(id => id !== 'BOT');
        if (room.roundSubmissions[humanId] !== undefined) {
          if (room.roundTimerHandle) clearTimeout(room.roundTimerHandle);
          resolveRound(room, pusherTrigger);
        }
      }
...
async function resolveRound(room, pusherTrigger) {
  ...
  if (dead.length > 0 || room.round >= 30) {
    ...
    setTimeout(() => {
      if (rooms[code] && room.status === 'playing') startRound(room, pusherTrigger);
    }, 3000);
  }
}
